...
void handle_push_to_b(t_stack_node **a, t_stack_node **b, int threshold, int *pushed, int *rotations)
{
    pb(a, b);
    (*pushed)++;
    *rotations = 0;
...
void handle_rotation_or_pivot(t_stack_node **a, int *pivot, int *pushed, int *rotations, int chunk_size)
{
...
void parse_b(t_stack_node **a, t_stack_node **b, int size)
{
    ...
    {
        if ((*a)->data <= pivot)
            handle_push_to_b(a, b, pivot - (chunk_size / 2), &pushed, &rotations);
        else
            handle_rotation_or_pivot(a, &pivot, &pushed, &rotations, chunk_size);
    }
}
